...
export default{
  ...
  computed: {
      ...mapState('userscrud', {
          ...
          data_table: state => state.data_table,
      ...
      singleSelect_table:{
        get() {
            return this.$store.state.userscrud.singleSelect_table
        },
        set(val) {
            this.$store.commit('userscrud/ASSIGN_SINGLE_SELECTED_TABLE', val)
        }
      },
      selected_table:{
        get() {
            return this.$store.state.userscrud.selected_table
        },
        set(val) {
            this.$store.commit('userscrud/ASSIGN_SELECTED_TABLE', val)
        }
      },
      page_table: {
          get() {
              return this.$store.state.userscrud.page_table
          },
          set(val) {
              this.$store.commit('userscrud/ASSIGN_PAGE_TABLE', val)
          }
      },
      search_table: {
          get() {
              return this.$store.state.userscrud.search_table
          },
          set(val) {
              this.$store.commit('userscrud/ASSIGN_SEARCH_TABLE', val)
          }
      }
  },
  watch: {
  ...
  methods: {
      ...mapActions('userscrud', ['Index','remove','show','removeAll']),
      create() {
        this.$store.commit('userscrud/SET_DIALOG', true)
      },
      Edit(id) {
        this.$store.commit('userscrud/SET_DIALOG', true)
        this.show(id)
